	...
	*/
	...
	public function produce_grid_feed($limit=25,$offset=0){
		...
		foreach($record_set['result_set'] as $record ){
			...
			$cat_record_set = $this->category_model->select_records('*',1,0,array('cat_id' => $record['category_id']));
			...
			$action = '<a href="javascript:void(0);" onclick="edit_record('.$record['lens_id'].')">Edit</a>  |  
